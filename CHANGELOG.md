...
- Show a run deliberately interrupted by a steering message as “direction updated” instead of presenting it as an app failure. Unexplained interruptions and real failures keep their existing error treatment.
- Includes the restart-safe checkpoints, fixed execution-folder binding, exact result identity and verifier status filtering from 1.2.29.
- **Pinned runtime** — this release binds Agentlas OS v1.2.44 at `1f6d64374502cfd5f8581ad3c1fb18691ed61b1d`; public asset `hephaestus-runtime-v1.2.44.tar.gz` has SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- Source readiness does not prove a public installer, installed update or long-running outcome; those remain separate verification gates.
...
- Evidence requirements are saved only after the classifier has finished without tool effects. Completion judgments also require settled verification execution; cancellation, timeout, unknown effects and late output cannot become a successful completion.
- Mandatory completion checks have a bounded three-minute model allowance while preserving the selected model, cancellation and the Goal's deadline. Uncertain verification effects remain visible to resume and startup safety checks.
- Automatically generated scope and evidence checks use the host's bound execution records and the requested outcome's proof requirements. These checks remain independently judged; missing evidence cannot be promoted to completion.
- A connection that throws before returning its execution promise now follows the same terminal and cleanup path as an asynchronous failure, instead of leaving the task marked active.
- Completed Antigravity clock and native-browser operations can supply Main-owned execution evidence to Goal checks. Evidence remains tied to the exact operation, Goal revision and settled run; completing a browser action alone does not prove publication or a successful business outcome.
- Older Goals that predate continuing-work settings can adopt those settings when the user explicitly confirms ongoing work while resuming the same paused or blocked Goal. The original objective, edited criteria, permission and budget remain intact; ambiguous instructions do not change the Goal's lifetime.
- Automatic Goal intake now receives its full one-minute allowance instead of being cut off internally at 30 seconds. Slow connected-model startup can finish without degrading the turn into an untracked one-off execution.

This release includes the 1.2.20–1.2.23 source changes below; their installer candidates were cancelled before publication after extended verification. It binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d, with runtime asset `hephaestus-runtime-v1.2.44.tar.gz` pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. Local ongoing work requires the app and computer to remain available.
...
This release includes the 1.2.20 and 1.2.21 source changes below; both installer candidates were cancelled before publication after extended verification. It binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d, with runtime asset `hephaestus-runtime-v1.2.44.tar.gz` pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. Local ongoing work requires the app and computer to remain available.
...
This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. Recovery of an already-modified older macOS installation is a separate delivery requirement; this release does not itself prove that recovery has reached those installations.
...
- Verified ongoing episodes reset their own no-progress streak, so repeated successful work does not stop on the fourth cycle. Unverified repetition still stops, and consumed budget remains recorded.
- Ongoing continuation context carries recent settled history with references to the full checkpoint, while preserving open work, uncertain effects and user constraints. Completed episode history no longer grows every model request without bound.
...
- 메모리는 매번 전부 다시 읽지 않습니다. 기존 프로필의 두 번째 시작은 이전 항목 스캔 0건이었고 창 로딩은 4.071초에서 2.165초로 줄었습니다. 진행 위치는 DB 내부 행 번호가 바뀌어도 유지되는 순서로 기록해, 압축·재구축 뒤 남은 메모리를 건너뛰지 않습니다. 약 130MB인 로컬 임베딩 모델은 마지막 사용 후 1시간 동안 유지해 보통 작업 중 재적재 멈춤을 피하고, 그 뒤 장시간 쓰지 않을 때만 반환합니다.
- 로컬 모델의 기본 선택은 모델 행에서 직접 켜고 끕니다. 가속을 끄면 진행 중인 로컬 실행을 먼저 멈추고 CPU 상태로 확실히 전환하며, 화면은 요청한 상태와 실제 상태를 구분합니다.
- 종료된 일회성 자동화가 다시 켜진 것처럼 보이면서 실제로는 예약되지 않던 상태를 막았습니다. 목록에는 `종료됨`을 표시하고, 실행 시간을 바꾸거나 `지금 실행`을 사용할 수 있습니다.
- 업데이트 준비가 45초 안에 끝나지 않으면 설치를 시작하지 않고 정상 종료로 돌아갑니다. 네이티브 업데이트 전에도 CLI 자식·CDP 브라우저·서비스·SQLite 정리를 완료해 창 없는 잔류 프로세스를 막습니다.
- Windows의 프로세스 조회 실패를 "이미 종료됨"과 구분합니다. 조회가 실패하면 정리 원장을 보존해 다음 주기에 다시 확인하고, 큰 목록은 128개씩 나눠 PowerShell 명령 길이 초과를 막습니다.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.11 — 2026-09-15

- Science: 연구자가 준 데이터 표가 드디어 논문에 **번호 붙은 표**로 들어갑니다. 지금까지는 원리적으로 불가능했습니다 — 표를 원고에 묶으려면 "채택된 화면 캡처"가 필수였고 그 캡처는 차트에만 자동으로 생겼습니다. 오너 데이터베이스 실측에서 원고 19판·결속 218건 중 표 결속은 0건이었고, AI 에게 남은 길은 숫자를 본문에 베껴 쓰는 것뿐이라 캡션도 번호도 출처도 붙지 않았습니다. 이제 표의 근거는 그림이 아니라 **정확한 행과 그 행을 만든 성공한 실행**입니다.
- Science: 표 전용 출처 관문이 생겼습니다. 영수증은 실제로 확인한 것만 적습니다(캡처·픽셀 검사는 하지 않으므로 적지 않습니다). 렌더러와 검증기가 같은 추출기를 쓰므로 영수증이 보증하지 않은 숫자는 논문에 찍힐 수 없습니다. 표는 편집이 곧 다른 숫자라 차트와 달리 파생 판을 승계하지 않습니다.
- Science: 가져온 데이터셋의 열 이름을 사람이 읽는 이름으로 찍습니다(전에는 `c3 (uM)` 같은 내부 키가 나왔습니다). 원고 작성 도구 설명이 표를 묶는 방법을 실제로 알려 줍니다 — 전에는 "그림·표·인용을 모두 선언하라"고만 하고 방법은 그림 것만 적혀 있었습니다.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.10 — 2026-09-15

- Antigravity: 연구가 시작조차 안 되던 것을 고쳤습니다. agy 자신의 하드코딩 보호 경계에 막힌 도구 호출 한 건이 실행 전체를 "승인 거부"로 죽이고, 권한으로는 열 수 없는 경계에 "권한을 올리세요"라는 해결책을 안내했습니다. 이제 런타임이 실제로 돌려준 거부 목록에 실린 것만 승인 거부로 보고, 보호 경계는 사실만 알린 뒤 같은 턴에서 계속합니다.
- 실행 폭주 차단기: 하네스를 잘못 짜도 기계를 태우지 못합니다. 한 런타임이 1분 안에 거듭 실패하면 잠시 실행을 멈추고 사람 문장으로 알립니다. 실패하는 두 런타임이 서로에게 요청을 넘기며 도는 경우에도 실제 실행은 분당 스물몇 회로 묶입니다. 성공은 세지 않으므로 바쁜 정상 작업은 영향받지 않습니다.
- 사용량 한도 기준을 하나로 모았습니다. 역할 풀 선택은 90%, 실제 실행 선택은 100% 로 갈려 있어서, 주간 사용량 90% 인 런타임이 설정 화면에서만 "한도 초과"로 지워졌습니다. 이제 한도가 실제로 소진됐을 때만 건너뜁니다.
- Science 0.1.31 을 함께 싣습니다: 프로젝트 폴더 화면 오른쪽 열이 통째로 잘리던 것, 연구가 멈춘 이유가 빈칸으로 나오던 것, 원고 화면을 PDF 처럼 보여 주기와 원고별 1단·2단 선택, 표 캡션과 여러 저자, 제출 점검 항목을 번호 목록으로, 오픈 웹·파일에서 전문 확보, 프로젝트 폴더 CSV 를 데이터 표로 가져오기.
- 넓은 표는 줄여서 끼우지 않고 가로로 돌립니다. 전에는 폭에 맞을 때까지 무한정 축소해 본문 9.7pt 에 표 4.2pt 까지 작아졌습니다.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.9 — 2026-09-14

- Science 서비스: 표 캡션을 "표 2. 제목"·"<표 2> 제목"·"Table 2: title" 형식으로, 표 위·아래 어디서든 읽습니다. 캡션이 없으면 출력 보고에 경고를 남깁니다. 한국어 원고는 표·그림·참조 라벨을 표/그림/식/절 로 찍습니다(HTML·PDF·DOCX). 기본 조판은 1단이며 원고별로 2단을 고를 수 있습니다. 초록·참고문헌·감사의 글에는 절 번호를 붙이지 않습니다.
- Science 서비스: 원고 속 차트는 결속된 스펙을 지면 기준으로 다시 그립니다 — 이미지 속 제목을 빼고(캡션이 제목), 축·범례 글자를 인쇄 크기로. 차트 제목을 저장할 때 분석 계획 id 같은 내부 식별자와 출처 꼬리를 지웁니다.
- Science 서비스: 수식이 든 원고도 DOCX 를 만듭니다. 분수 안 첨자 같은 수식은 Word 수식으로 바뀌고, 바꾸지 못한 수식은 LaTeX 글자로 실리며 흠으로 표시됩니다(제출 번들은 흠 없는 빌드를 요구). 조판 경고가 붙은 PDF 도 내려받을 수 있고, 출력 기록에 실패·경고 사유를 사람 문장으로 보입니다.
- Science: 조판 미리보기는 210mm 종이를 화면 폭에 맞춰 축소해 PDF 와 같은 단 폭으로 보입니다. 편집한 차트 판도 데이터가 같으면 실행 계보를 이어받아 원고에 넣을 수 있고, Vega 차트는 검증 때 화면 없이 캡처합니다("지금 검증"). 원고 실패 칩과 "새 원고 시작" 단추가 진행 상태를 따르고, 프로젝트 폴더 화면 본문이 절반으로 잘리던 것을 고쳤습니다.
- Science·Work·One: 작업 블록 줄 규칙을 한 벌로 모았습니다(연속 생각 병합·이름 없는 도구 이벤트 제외·경로 축약). 방금 보낸 내 말풍선은 답이 오는 동안 화면 위쪽에 고정됩니다.
- Science 서비스: 규정 페이지·전문의 문자 집합을 판별하고(EUC-KR 등), 페이월 안내를 본문으로 받지 않으며, 같은 도구 거절이 반복되면 연구자에게 묻도록 안내합니다. 통계 엔진 1.11.0: 분석 결정 원장(decisionLog)과 선형·로지스틱 회귀의 HC3·부트스트랩·순열 동반. 제출 번들에 연구 디렉터 프롬프트 전문과 해시를 첨부합니다. 연구 디렉터 1.24.8.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.8 — 2026-09-14

- Science 서비스: 제목·색·축을 고친 차트 판(v2…)도 데이터가 같으면 실행 계보를 이어받아 원고에 넣을 수 있습니다(전에는 편집된 판은 검증 자체가 불가능해 "원고 연결 불가"로 남았습니다). 데이터가 바뀐 판은 사람 문장으로 거절합니다.
- Science 서비스: Vega 차트는 검증할 때 채택된 캡처가 없으면 화면 없이(Vega→SVG→PNG, 1600px) 찍고 검증합니다 — 차트를 열 필요가 없습니다. 결과·그림 화면에 "지금 검증" 단추와 아직 안 되는 이유를 보여 줍니다.
- Science·Work·One: 작업 블록 줄 규칙을 한 벌(shared/turn-activity-rows.ts)로 모았습니다. 연속된 생각은 한 줄로 합쳐 누적 시간을 보이고, 이름 없는 도구 이벤트(런타임 상태 하트비트)는 도구 줄이 되지 않으며("도구 unknown" 제거), 도구 줄의 경로·명령은 사람이 읽는 한 줄로 줄입니다.
- Science: 원고 실패 칩은 새 턴이 돌면 "이어가는 중"으로 바뀌고 턴이 끝나면 다시 판정합니다. "새 원고 시작"은 작업 중 비활성입니다. 최근 대화 레일은 대화가 하나면 숨깁니다. "새 원고 시작" 모달이 화면 언어를 따릅니다.
- Science 서비스: 저널 규정 페이지와 전문의 문자 집합을 판별합니다(HTTP charset → meta → EUC-KR 재시도). 깨진 페이지는 거절하고 수동 입력 길을 엽니다. 페이월·로그인 안내는 본문으로 받지 않습니다. 같은 도구 거절이 한 턴에 세 번이면 재시도 대신 연구자에게 묻도록 안내하고, 청사진 거절 24종에 사람 문장을 붙였습니다(실패 문장 59종).

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.7 — 2026-09-14

- Science 서비스: 연구 루프 중에 모델을 바꾸면 다음 턴부터 새 모델로 이어집니다(전에는 에피소드 경계를 기다리다 루프 만기까지 적용되지 않았습니다). 안내에 적용 시점을 적고 "지금 적용"(현재 턴 중지 → 새 모델로 연구 계속)을 둡니다. 앱 재시작 뒤 재개도 바꾼 모델을 유지합니다.
- Science 서비스: 각주를 지원합니다 — 본문 [^key], 정의 줄 "[^key]: 본문". PDF 는 쪽 아래 가로줄 밑 각주, 미리보기는 본문 끝 각주 목록, DOCX 는 미주. 저자 국문(영문) 병기·소속 병기·교신저자·ORCID 를 지면에 찍습니다(render 도구 metadata.authors 의 nameLatin·affiliationsLatin·orcid).
- Science 서비스: 도구 실패마다 사람 문장과 다음 행동을 붙입니다(주장 원장·문헌·저널·가설 계열 24종, 나머지는 일반 문장). 코드 뒤의 해시·id 목록은 details 로 접습니다. 핵심 도구 69개 설명에 한국어 키워드를 넣어 한국어 도구 검색이 걸리게 합니다.
- Science: 멈춘 연구 루프에 이유와 다음 행동을 보여 줍니다(예산 소진·기한 만료·연구자 결정 대기·모델 없음·계약 변경·사용자 중지·앱 종료). 그림 표시 실패에 호스트 사유를 함께 적습니다.
- 연구 디렉터 계약(플러그인 1.24.5): 도구 지도(도구가 필요하면 먼저 검색, 여섯 묶음, 거절의 nextAction 을 따를 것)를 넣었습니다.
- Work: 컴퓨터 유즈 등 MCP 도구 연결이 실행 중 끊겨도 프록시가 같은 자리에서 다시 붙어 실행이 이어집니다(전엔 그 턴의 모든 호출이 "연결 안 됨"으로 실패).
- Work: 질문 카드의 답은 데스크탑·모바일·다른 창 어디서 보내도 같은 규칙으로 받고, 목표가 멈춘 이유(같은 결과 반복·대기 기한·예산 소진)를 사람 말과 다음 행동으로 보여 줍니다.
- Work·One: 실행 중에 모델을 바꾸면 언제 적용되는지(다음 메시지·재개부터)와 바로 바꾸는 방법(중지 후 다시 보내기)을 안내합니다. 앱 재시작 뒤 자동으로 이어지는 작업이 원래 모델로 이어갈 때, 그 사이 바꿔 둔 모델이 있으면 어떤 모델로 이어가는지 대화에 알립니다.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.2.6 — 2026-09-14

- Science 서비스: 넓은 표를 두 단 지면에서 한 단에 눌러 넣지 않고 두 단에 걸치게 하고, "Table 1: …"·"**Table 2.** …" 캡션 줄을 표 캡션으로 읽습니다(전에는 "Table 1: Table 1"이 찍혔습니다). 캡션 없는 표는 자기 번호를 캡션으로 쓰지 않습니다.
- Science 서비스: 원고마다 1단 또는 2단 조판을 고를 수 있습니다. 화면의 "PDF 설정"과 render_science_manuscript 의 column_count 가 같은 값을 쓰며, 저널 프로파일 조판 위에 덮습니다.
- Science 서비스: 일반 렌더는 저널 결속과 무관하게 항상 "DRAFT — NOT FOR SUBMISSION" 표식이고, 워터마크 없는 PDF 는 검증을 통과한 제출 번들에서만 나온다는 규칙을 렌더 도구가 설명합니다(새 원고 id 는 원장·검증·내보내기를 새로).
- Science 서비스: 저널 정체성 확인 도구(confirm_journal_identity)와 그림 결속 사슬·원고 표식 문법 안내(1.2.5 에 실린 항목의 핀 갱신).
- Work: 자동 목표가 "변화 없음"을 숫자만 바꿔 반복하던 것을 3턴에서 멈추고, 프로토콜 토큰이 화면에 새지 않게 합니다. 사용자에게 묻는 회차는 목표 순환을 끝내고, One 은 답이 전송되지 않은 이유를 말합니다.
- 목표 계약: 게임·앱·UI 산출물은 그래픽 품질이 합격 기준입니다. 플레이스홀더 수준의 그래픽은 완료가 아니라 실패입니다(오너 결정).
- Work: 질문 카드의 답이 "만료된 질문"으로 버려지던 것을 고칩니다. 질문 뒤에 사용자 답이 없는 한 답을 받습니다(선택해도 전송되지 않던 문제). 질문을 낸 턴은 목표 연속 실행을 멈춰 답이 전송되게 합니다.
- Work: 컴퓨터 유즈 같은 MCP 도구 연결이 실행 중 끊기면 사람에게 한 번 알리고 앱 로그에 사유를 남기며, 모델은 반복 호출 대신 정리합니다. 채팅·프로젝트 기억란에 내부 표식이 보이던 것을 지웁니다.
...
- 작업공간 첫 화면을 1.2.0 이전의 프로젝트 목록 화면으로 되돌립니다. 1.2.0의 "문장을 적으면 프로젝트와 대화가 함께 준비되는" 흐름은 보낼 때마다 문장을 이름으로 한 새 프로젝트를 만들어 대시보드를 어지럽혔습니다(오너 결정 2026-09-13). 프로젝트를 먼저 고르거나 연결한 뒤 대화를 시작합니다.
- 프로젝트 만들기 화면의 "프로젝트 도구 · 제거" 버튼이 눌리지 않던 것을 고칩니다. 도구 행이 드래그용으로 포인터를 잡아 두어 버튼 클릭이 행에 삼켜졌습니다. 프로젝트 상세에서 도구를 저장한 직후 숨겨진 배경 칸이 카드로 되살아나던 것도 함께 고칩니다.
- 프로젝트 상세의 "Hub 추천" 버튼을 검정 바탕·흰 글자로 바꿔 눈에 띄게 합니다.
- 대시보드 "진행 중인 일" 카드의 프로젝트 이름이 길면 여러 줄로 커지던 것을 한 줄 말줄임으로 고정합니다.
- 설정의 "Agentlas Local · 연결" 버튼이 설정 화면 자신으로 가던 것을 로컬 모델 화면으로 가게 고칩니다.
- 로컬 모델 실행 엔진 설치가 실패하면 "완료하지 못함"만 보이던 것을 고쳐 사유를 함께 보여 줍니다(예: 엔진 검증용 내장 Node 사용 불가). 사유는 앱 로그에도 남고, 내장 Node 재검증 제한 시간을 60초로 늘립니다.
...
- 로컬 모델이 대화 용량을 넘기면 조용히 잘라내는 대신 먼저 재고, 선택적인 화면 제작 안내만 뺀 채 한 번 더 시도하고, 그래도 안 되면 정직하게 알립니다. "도구를 쓸 필요 없다"고 적은 요청이 도구 안내 때문에 실패하던 문제를 고칩니다.
- One에 말로 자동화를 시킬 수 있습니다. "매일 아침 9시에 오늘 할 일 물어봐 줘"처럼 적으면 일정이 등록되고, 같은 대화에서 시간을 바꾸거나 멈추고 다시 켤 수 있으며, 결과는 원래 대화로 한 번만 돌아옵니다. 팀을 꾸려 달라고 하면 단톡방까지 만들어 둡니다.
- 작업공간 첫 화면에서 바로 요청을 적으면 프로젝트와 대화가 함께 준비됩니다.
- 사이드바를 정리했습니다. 사이트 메뉴가 사라지고 에이전트 그룹(만들기·에이전트 관리·에이전트 업로드·에이전트 마켓·에이전트 그래프)으로 모입니다.
- 에이전트 마켓과 대시보드 카드에서 보안 등급과 싱글·멀티 글자를 걷어내고 우측상단 아이콘 하나로 종류를 표시합니다. 조직도의 멀티·싱글 탭이 클라우드 팀까지 올바르게 나뉩니다.
- 내장 브라우저가 진짜 브라우저처럼 얇아집니다. 저장된 로그인이 없으면 첫 화면 위에 작은 배너로 가져오기를 안내하고, 온보딩의 브라우저 가져오기 단계는 없어집니다. 비밀번호·연락처 자동 입력, 방문 기록, 큰 파일 다운로드가 작업 예산 안에서 동작합니다.
- 문서(PDF·DOCX·XLSX·PPTX·HWPX) 패널의 버전·해시·단계 설명을 걷어내고 한 줄 도구막대와 작은 메뉴로 통일했습니다. 편집 초안은 미리보기가 다시 떠도 보존됩니다.
...
- 로컬 모델과 내 API 키로 도는 실행이 도구를 8번 주고받으면 거기서 끊기고 실행 전체가 버려지던 것을 고칩니다. 파일 하나 읽고 고치고 확인만 해도 서너 번이라 정상적인 작업이 실패로 처리되고 있었습니다. 이제는 횟수가 아니라 같은 도구를 같은 값으로 반복하는지로 판단합니다.
...
- Goal 원문과 완료 기준을 임의 글자 수로 자르거나 거절하던 저장 제한을 제거하고, 종료된 Goal 결속과 계약이 남아 있으면 새 요청을 등록하기 전에 함께 정리합니다. 이미지·파일·폴더 등 첨부 단독 요청은 분류 장애로 표시하지 않고 일반 실행합니다.
- 지원 로그에 프롬프트나 스택을 남기지 않으면서 분류와 Goal 등록 중 어느 단계에서 실패했는지 구분합니다.
- One과 Work에서 장문 요청을 자르지 않으며, 첨부가 있는 요청은 임시 파일을 잃는 자동 후속 패스로 넘기지 않고 현재 실행에서 처리합니다.
- Science 내장 분석 플러그인(통계·천문·물리·지구·소재·고생물·비교유전체·경제)이 포장된 앱에서 이름만 같은 다른 폴더를 먼저 찾아 전부 실행 불가로 떨어지던 문제를 고칩니다.
- 목표가 끝나면 대화 화면이 그 사실을 즉시 받아 칩과 목표 토글을 정리합니다. 다른 대화에 갔다 와야 사라지던 문제를 고칩니다.
