...
// busy/streaming 에이전트 드래프트는 저장하지 않는다 — 재진입 시 attach가 main 버퍼에서
// 라이브 버블을 재구성하므로, 여기 남기면 중복 버블이 된다. 단, 이미 Main steering 큐로
// 접수된 user turn은 실행 중이어도 보존해야 한다. 이 메시지는 busy/streaming 플래그가 없다.
...
export function saveChatViewSnapshot(chatId: string, snap: ChatViewSnapshot): void {
  const cleaned: ChatViewSnapshot = {
    messages: snap.messages.filter((m) => m.role === "user" || (!m.busy && !m.streaming)),
