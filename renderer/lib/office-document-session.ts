...
export function reduceOfficeDocumentSession(state: OfficeDocumentSession, action: OfficeDocumentSessionAction): OfficeDocumentSession {
  ...
  if (action.type === "change-draft") {
    const originalValue = officeSelectionValue(state.selection);
    if (!state.selection || originalValue === null || !state.activeRevision || state.conflict) return state;
    ...
    return {
      ...state,
      draftSequence,
      pendingEdit: { selection: state.selection, originalValue, replacementValue, draftSequence, delivery: "draft" },
    };
  }
