...
export function BrowserAutofill({ open, mode, target, ko, onClose, onPrepareOverlay, onOverlayClosed }: {
  ...
  useEffect(() => {
    ...
    return () => { active = false; clearPlaintext(); };
  }, [open, target?.viewId, target?.taskScopeId, ko, onPrepareOverlay, clearPlaintext]);
  ...
  const saveCredential = async () => {
    ...
    clearPlaintext();
    setEditing(null); setNotice(ko ? "비밀번호를 안전하게 저장했습니다." : "Password saved securely."); await refresh();
  };
  const saveContact = async () => {
    ...
    clearPlaintext();
    setEditing(null); setNotice(ko ? "연락처를 안전하게 저장했습니다." : "Contact saved securely."); await refresh();
  };
  const fill = async (id: string) => {
    const client = api(); if (!client || !target) return;
  ...
  const remove = async (id: string) => {
    ...
    if (result.ok) { setRemovePending(null); setEditing(null); await refresh(); }
  ...
  return <div ref={root} className={`${menu.panelPopover} ${styles.panel}`} role="dialog" tabIndex={-1}
    ...
    <div className={styles.header}><strong>{ko ? (mode === "passwords" ? "비밀번호" : "연락처") : mode === "passwords" ? "Passwords" : "Contacts"}</strong><button type="button" onClick={close}>{ko ? "닫기" : "Close"}</button></div>
    ...
    {!unavailable && rows.map((item) => <div className={styles.item} key={item.id}>
      ...
      {removePending === item.id ? <div className={styles.actions}><button type="button" onClick={() => setRemovePending(null)}>{ko ? "취소" : "Cancel"}</button><button type="button" onClick={() => void remove(item.id)}>{ko ? "삭제 확인" : "Delete"}</button></div>
        : <div className={styles.actions}><button type="button" disabled={!target} onClick={() => void fill(item.id)}>{ko ? "채우기" : "Fill"}</button><button type="button" onClick={() => mode === "passwords" ? beginCredential(item.id) : beginContact(item.id)}>{ko ? "전체 교체" : "Replace"}</button><button type="button" onClick={() => setRemovePending(item.id)}>{ko ? "삭제" : "Delete"}</button></div>}
    </div>)}
    {!unavailable && rows.length === 0 && <p className={styles.notice}>{ko ? "저장된 항목이 없습니다." : "No saved items."}</p>}
    {!unavailable && editing === null && <button className={menu.panelMenuRow} type="button" onClick={() => mode === "passwords" ? beginCredential("new") : beginContact("new")}>{ko ? "새로 저장" : "Save new"}</button>}
    {!unavailable && editing !== null && <div className={styles.form}>
      ...
      <label>{ko ? "이름" : "Label"}<input ref={label} maxLength={120} autoComplete="off" /></label>
      {mode === "passwords" ? <>
        <label>{ko ? "사이트" : "Site"}<input ref={origin} maxLength={2048} inputMode="url" autoComplete="off" /></label>
        <label>{ko ? "사용자 이름" : "Username"}<input ref={username} maxLength={512} autoComplete="off" /></label>
        <label>{ko ? "비밀번호" : "Password"}<input ref={password} type="password" maxLength={4096} autoComplete="new-password" /></label>
      </> : CONTACT_FIELDS.map((field) => <label key={field}>{ko ? ({ name: "이름", email: "이메일", phone: "전화번호", organization: "회사", addressLine1: "주소", addressLine2: "상세 주소", city: "도시", region: "지역", postalCode: "우편번호", country: "국가" }[field]) : field}<input ref={node => { if (node) contactInputs.current[field] = node; else delete contactInputs.current[field]; }} maxLength={512} autoComplete="off" /></label>)}
      <div className={styles.formActions}><button type="button" onClick={() => { clearPlaintext(); setEditing(null); }}>{ko ? "취소" : "Cancel"}</button><button type="button" onClick={() => void (mode === "passwords" ? saveCredential() : saveContact())}>{ko ? "안전하게 저장" : "Save securely"}</button></div>
    </div>}
