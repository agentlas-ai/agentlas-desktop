...
import { IconClose, IconCpu, IconRefresh, IconSearch } from "@/components/Icon";
import type { HuggingFaceModelFile, HuggingFaceRepositoryInspection, HuggingFaceSearchResult } from "@shared/local-model-hub";
...
export function HuggingFaceModelBrowser({ ko, onInstalled, onOperationStarted }: { ko: boolean; onInstalled: (packageId: string) => void; onOperationStarted?: (id: string) => void }) {
  ...
  const [operation, setOperation] = useState<{ id: string; packageId: string; fileName: string } | null>(null);
  ...
  const install = async (file: HuggingFaceModelFile) => {
    ...
    try {
      ...
      const id = crypto.randomUUID(); onOperationStarted?.(id); setOperation({ id, packageId: model.packageId, fileName: model.fileName });
      await api.installModelPackage({ packageId: model.packageId, operationId: id });
      if (request.cancelled || !mounted.current) return;
      close(); onInstalled(model.packageId);
    } catch (failure) { setError(failure instanceof Error && failure.message === "cancelled" ? (ko ? "다운로드를 취소했습니다." : "Download cancelled.") : (ko ? "다운로드를 완료하지 못했습니다. 연결과 저장 공간을 확인하고 다시 시도하세요." : "Download did not finish. Check your connection and disk space, then retry.")); }
    finally { installing.current = false; intent.current = null; setRegistering(false); setOperation(null); }
  };

  const cancel = async () => {
    if (intent.current) intent.current.cancelled = true;
    if (operation) {
      try { await ipc()?.localModelHub.cancelOperation({ operationId: operation.id }); }
      catch { setError(ko ? "중지 상태를 확인하지 못했습니다. 다시 시도하세요." : "Could not confirm cancellation. Try again."); }
    }
  };
  ...
  return <section className={styles.browser} aria-label={ko ? "Hugging Face 모델 탐색" : "Browse Hugging Face models"}>
    ...
    <div className={styles.grid} aria-busy={loading}>
      {result?.models.map(model => <button key={model.repository} type="button" data-hf-repository={model.repository} className={styles.card} onClick={event => { opener.current = event.currentTarget; setSelected(model.repository); }}>
        <span className={styles.modelMark}><IconCpu size={22} /></span><span className={styles.publisher}>{model.author ?? model.repository.split("/")[0]}</span>
    ...
    {registering && !selected && <div className={styles.downloadStatus} role="status"><span>{ko ? "파일의 원본 버전 확인 중…" : "Checking the exact source revision…"}</span><button type="button" onClick={() => void cancel()}>{ko ? "중지" : "Stop"}</button></div>}
    ...
    {selected && <div className={styles.backdrop} onPointerDown={event => { if (event.target === event.currentTarget) close(); }}><div ref={dialog} className={styles.detail} role="dialog" aria-modal="true" aria-labelledby="hf-model-detail-title">
      ...
      {inspecting ? <p role="status">{ko ? "원본 파일 정보를 확인하는 중…" : "Checking source file metadata…"}</p> : inspection && <>
        ...
        {inspection.stale && <p role="status">{ko ? "저장된 파일 정보입니다. 다운로드 전에 원본 버전을 다시 확인합니다." : "These are saved file details. The exact source revision is rechecked before downloading."}</p>}
        <div className={styles.files}>{inspection.files.map(file => <div key={file.fileName} className={styles.file}><div><strong>{file.fileName}</strong><span>{file.quantization ?? "GGUF"} · {size(file.byteLength)}</span>{!file.downloadable && <span>{inspection.gated === true ? (ko ? "원본 저장소에서 접근 승인이 필요합니다." : "Access approval is required at the source.") : (ko ? "이 파일은 현재 다운로드 대상으로 지원하지 않습니다." : "This file is not currently supported for download.")}</span>}<details><summary>{ko ? "파일 정보" : "File information"}</summary><code>{file.sha256 ?? "SHA-256 unavailable"}</code>{file.reasonCodes.length > 0 && <code>{file.reasonCodes.join(", ")}</code>}</details></div><button type="button" disabled={!file.downloadable || registering || operation !== null || !inspection.revision} onClick={() => void install(file)}>{ko ? "다운로드" : "Download"}</button></div>)}</div>
      ...
      {registering && <div className={styles.downloadStatus} role="status"><span>{ko ? "원본 버전 확인 중…" : "Checking source revision…"}</span><button type="button" onClick={() => void cancel()}>{ko ? "중지" : "Stop"}</button></div>}
