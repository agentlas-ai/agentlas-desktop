...
export function ToolObservation({ toolName, callId, detail, args, result, locale }: {
  toolName: string; callId?: string; detail?: string; args?: string; result?: string; locale: "ko" | "en";
}) {
  ...
  return <section className={styles.observation} aria-label={ko ? "도구 관측 결과" : "Tool observations"}>
    ...
    {(callId || detail || args || result) && <details className={styles.raw} onToggle={(event) => setRawOpen(event.currentTarget.open)}>
      <summary>{ko ? "도구 원문" : "Original tool details"}</summary>
      {rawOpen && <>
        {[[ko ? "호출 ID" : "Call ID", callId], [ko ? "인자" : "Arguments", args], [ko ? "결과" : "Result", result], [ko ? "상세" : "Detail", detail]].map(([label, value]) => value ? <div key={label}>
