...
- **2026-08-12 · v0.9.90 — navigation keeps its evidence, and Goal stops drifting** — Returning to Dashboard reuses the last verified account, credit, project, runtime, usage, quest, Hub, and organization projections while fresh reads reconcile in the background, so a signed-in connected workspace no longer flashes as logged out and disconnected. Goal is an explicit, normally-off campaign whose first request freezes one local objective and five engineering acceptance criteria; ordinary chat and steering cannot replace it. A direction typed while the model works is staged above the composer and joins the Main-owned steering queue only through the explicit adjustment action. That queued turn, event-grounded tool/file/image feedback, completed answers, and attached screenshots survive Dashboard and menu round trips, and One exposes concrete outputs in a compact rail without inventing progress percentages. This release binds Agentlas OS v1.1.111 at ee1f23911f378b6d521e64d89713c4ef15eb38e9. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
