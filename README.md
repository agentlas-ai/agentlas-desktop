...
- **2026-08-13 · v1.0.2 — One sees every connected runtime, and steering sends once** — One now builds its picker from every locally connected provider and persists the exact provider/model chosen for that conversation; Claude Code Fable is visible even when a CLI inventory is incomplete. During a running turn, the same round composer control sends the next instruction immediately without stopping the model or presenting a second confirmation step. This release binds Agentlas OS v1.2.0 at 8b3f8bcffdfc57bf4991ed6e43d153d9230ea186. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
