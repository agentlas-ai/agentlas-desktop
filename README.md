...
- **2026-09-26 · v1.2.44 — One 에 메일함, 새 사용자 첫 설정, 새 기능 안내** — One 레일의 "메일" 탭에서 받은 메일·답장 기다림·보낸 메일·임시보관·보관함·연락처를 보고, One 이 메일을 읽고 초안을 쓰고(자동 모드에서는) 보냅니다. One 마다 만들 때 정하는 영구 주소가 생기고, Agentlas 디렉터리 등재·내 도메인(DNS 레코드 카드)·에이전트 간 메일을 쓸 수 있습니다. 메일 발송 거절(붐빔·일일 한도·정지)은 사람 문장으로 알려 주고, 메일 삭제는 앱 안 확인창으로 묻습니다. One 편집은 프로필·메일·디렉터리·내 도메인 탭으로 나뉘고, One 프로필은 로그인 계정마다 하나입니다. 새 사용자는 로그인→이름→Chrome→AI→말투·원칙→메일 순서의 첫 설정을 거치고, 기존 사용자는 업데이트 뒤 계정마다 한 번 새 기능 안내를 봅니다. One 이 팀원의 세션을 열어 일을 맡기고 결과를 돌려받으며, 승인 대기 배너의 "열기"는 기다리는 그 대화로 갑니다. 폰은 One 메일함·프로필을 함께 보여 주고, Work 프로젝트 에이전트는 요금제와 무관하게 추가할 수 있습니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.51 at 4ca8fd28d5ad9a92cbe33b39d05bcff8febe7448. Its public runtime asset `hephaestus-runtime-v1.2.51.tar.gz` is pinned at SHA-256 `0f95c8844fd9793285ac41031eea1ed4f98dba3a10707918f7cdcd74230bc1f3`.
- **2026-09-25 · v1.2.43 — One·Work 에 AGI, 목표는 스스로 끝까지, Agentlas AI 도 같은 도구로** — 작성창의 AGI 단추로 목표마다 오케스트레이터를 켜면 대시보드 모델 순서와 토큰 한도 안에서 짧은 판단 호출로 깨어나 멈춘 일을 이어갑니다(오너가 멈춘 목표·승인은 넘지 않습니다). 목표 완료는 AI 가 나눈 하위목표→전략→최종목표로 롤업되고, 조건에 적힌 파일은 앱이 직접 읽어 확인합니다. 한 번에 끝나는 요청은 평범한 턴으로 남고, 목표 턴은 끝날 때마다 검증·다음 단계 예약·이유 있는 정지 중 하나로 이어지며, 질문으로 끝나면 답을 기다렸다가 이어갑니다. 멈춘 목표는 오너가 다시 켤 때까지 멈춰 있고 재개는 누른 즉시 반영됩니다. Agentlas AI(크레딧)도 One·Work 에서 CLI 와 같은 도구·목표 계약을 받고, 자동화는 실제 바깥 효과만 진전으로 셉니다. 엔진 도구가 읽기 전용으로 표시돼 Work 가 승인을 묻지 않고, 새 프로젝트에 signing/·credentials/·.env.example 이 생기지 않습니다. Agent Hub 는 무료로 바뀌었고 사이드바 크레딧은 작은 에너지 막대로 보입니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.51 at 4ca8fd28d5ad9a92cbe33b39d05bcff8febe7448. Its public runtime asset `hephaestus-runtime-v1.2.51.tar.gz` is pinned at SHA-256 `0f95c8844fd9793285ac41031eea1ed4f98dba3a10707918f7cdcd74230bc1f3`.
...
- **2026-09-24 · v1.2.41 — 목표 자동화가 스스로 전략을 고치고, 스스로 고른 보류가 규칙이 되지 않게** — 전권을 준 진행형 목표의 대화가 고친 자동화는 전략 변경이 대기 없이 반영되고, 정말 내 결정이 필요한 것은 화면에 뜹니다. 무인 실행이 스스로 정한 보류·한도는 영구 규칙으로 남지 않고 다음 실행에는 휴리스틱으로 보이며, 다음 실행은 지난 실행의 기록된 사실을 보고 다시 판단합니다. 할 일이 있는데 쉰 실행은 완료로 치지 않습니다. 이전 작업 확인은 대화체 답에서도 읽히고, 재시작으로 끊긴 조회 단계는 그냥 다시 돕니다. 목표 대화에서 목표치를 바꾸면 다음 멈춤에 목표 자체가 갱신됩니다. 브라우저 가져오기 창의 겹침도 고쳤습니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-24 · v1.2.40 — 자동화는 지금 쓰는 모델로, 헛도는 실행은 멈추고, 답은 내 언어로** — 비서가 만든 자동화는 만들 때 골라져 있던 모델 대신 보고하는 대화의 모델(없으면 워커 기본값)로 돕니다. 직접 고른 모델은 그대로 두고, 한도에 걸리거나 계속 헛돌면 그 실행만 워커 목록의 다른 모델로 넘깁니다. 같은 페이지만 되풀이해 보는 실행은 이유와 함께 멈추고, 결과 판정은 오케스트레이터 모델이 먼저 합니다. 게시물 언어 규칙이 있어도 나에게 하는 답과 보고는 앱 언어로 옵니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-24 · v1.2.39 — 목표는 직접 보고 이어가고, 빈 값·거절 이유로 막히지 않게** — 확인이 필요한 목표는 앱이 강제하는 읽기 전용 브라우저로 페이지를 새로 보고 이어갑니다(창에 없는 대화·Codex 포함). 빈 항목 때문에 요청이 거절되던 계열을 입구에서 없앴고, 거절은 이유와 다음 행동을 보여 줍니다. Work 시작·모델 변경·목표 재개, 자동화 저장, 내 API 키로 쓰는 모르는 모델도 더는 막히지 않습니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-23 · v1.2.38 — 목표가 도는 대화에 보낸 지시가 거절되지 않게** — 모델을 고른 One 팀·작업 대화에서 1.2.33 부터 메시지가 거절되던 문제를 고쳤고, 목표가 도는 중에 보낸 지시는 그 목표의 방향 수정으로 바로 들어갑니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-23 · v1.2.37 — 설치한 플러그인은 할 일에 맞춰 자동으로, 만든 이미지는 바로 올릴 수 있게** — One·Work 모든 턴에서 설치된 플러그인(도구·스킬)을 할 일과 맞춰 골라 붙이고, 판정이 실패해도 관련 플러그인을 놓치지 않습니다. Codex·Antigravity·앱이 만든 이미지는 작업 폴더로 복사돼 바로 올릴 수 있고, Work 장기 목표도 One 처럼 막히면 다시 계획하고 한도에 걸리면 다른 모델로 넘어갑니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-23 · v1.2.36 — 막히는 목표 없음, 기억은 영어로 찾고 원문으로 보여 줌** — 목표가 앞 단계 결과를 모를 때 멈추지 않고 직접 확인한 뒤 이어가거나 다시 하거나 스스로 나중에 다시 확인합니다. 예전 버전이 남긴 막힌 목표는 처음 켤 때 정리됩니다. 기억은 영어로 검색되고 원문 그대로 보여 주며, 기존 기억은 한가할 때 조금씩 옮겨집니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-23 · v1.2.35 — 윈도우 연결은 로그인만, One 목표는 막히지 않게** — 윈도우에서 연결을 누르면 claude·codex·grok 의 공식 실행파일을 받아 검증한 뒤 그대로 씁니다(npm·Node 불필요, 회사 프록시·인증서 자동, 사용자 이름이 한글이어도 됨). One 의 목표 표시는 한 줄로 접히고, 재개는 한 문장과 버튼 하나이며, 자동화가 연결된 목표도 더는 막다른 길로 끝나지 않습니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-23 · v1.2.34 — 윈도우 첫 연결 설치 복구** — 윈도우에서 연결을 눌러도 CLI 설치가 되지 않고 연결 화면으로 되돌아가던 문제를 고칩니다. 앱 안의 Node 실행파일을 밖으로 복사하지 못해도 설치가 그대로 이어지고, 네트워크 일시 장애에는 자동으로 다시 시도하며, 실패해도 앱 재설치를 안내하지 않습니다. Source readiness does not prove a public installer or installed update; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
...
- **2026-09-20 · v1.2.32 — Science 완전 자율(베타, macOS)·레퍼런스 문서 양식, 상주 프로세스 누수 수리** — Agentlas Science 0.1.34 서비스를 싣습니다: 챗 입력창 실행 모드의 Full Autonomy (Beta)는 macOS에서만 제공되며, 레퍼런스 DOCX/HWPX의 문서 양식으로 원고를 씁니다. Science 턴과 턴 뒤 기억 검토가 끝난 뒤 CLI 프로세스가 12시간씩 남던 누수를 고쳤습니다(실측: 40분에 8개 → 0개). One·Work에는 완전 자율이 들어가지 않습니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-19 · v1.2.31 — Full access 및 장기 Goal 오류 복구 수정** — Full access는 첫 페이지가 아직 없는 상태에서도 Main의 실행 권한을 확인해 브라우저 코드 승인을 자동 통과합니다. 종료가 확인된 읽기·브라우저 도구 실패는 정리 완료된 실패로 기록하므로, ongoing Goal이 오류 한 번에 영구 차단되지 않고 다음 대기·실행 주기로 이어집니다. 1.2.30의 실제 브라우저 콜백 종료 확인과 스티어링 표시 수정도 포함합니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-19 · v1.2.29 — 목표 대기·복귀와 결과 검증 수정** — 폴더를 지정하지 않은 One 실행도 실제 작업 경로를 유지하고, 확인된 중단 이력 뒤에는 새 안전 체크포인트만 재개합니다. 내부 상태 안내와 실제 도구 사용을 구분하며, 같은 실행 결과의 중복 저장을 막습니다. 기존의 증명되지 않은 체크포인트는 자동 재실행하지 않습니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.

- **2026-09-19 · v1.2.28 — 실행 권한과 승인 화면 연결 수정** — 브라우저 자식 프로세스까지 Full access를 전달하고, 승인 요청을 원래 대화에 묶습니다. One의 요청이 Work 화면에 나타나거나 Agent Market·Agent Graphs 본문을 밀어내지 않도록 수정합니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-19 · v1.2.27 — Science 도구 등록 복구** — Scientific Skills의 지원 플랫폼·파일 무결성·도구 분류 선언을 바로잡아 절차 검색 도구가 설치 후 등록되도록 합니다. Science 인터페이스는 Desktop 의존성 버전과 별도로 서명된 확장 카탈로그를 통해 업데이트됩니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-18 · v1.2.26 — 설치 업데이트 뒤에도 자동화를 정상 복구합니다** — 1.2.25 실설치 검증에서 드러난 구형 키체인 실패 잠금을 버전 단위 1회 복구로 교정했습니다. 같은 버전의 반복 호출은 계속 차단해 무한 재시도를 막고, 새 버전만 서명된 격리 경로를 한 번 검증합니다. One 화면 안에도 업데이트·재시작 버튼을 직접 표시해 전용 레일 뒤에 가려지지 않습니다. 1.2.25의 단톡방·이미지·Full Access·스티어링·One/Work/Science 화면 소유권 수정도 모두 포함합니다. Agentlas Science 0.1.33을 유지합니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
- **2026-09-18 · v1.2.25 — 단톡방·이미지·화면 전환을 안정화합니다** — One 태스크포스 단톡방을 반복해서 눌러도 Task 상세로 바뀌거나 대화가 사라지지 않습니다. One과 Work의 브라우저 캡처·생성 이미지를 대화에 영구 저장해 재시작 뒤에도 표시하고, 결과 패널은 닫혀 있어도 이미지 열기 요청에 다시 열립니다. One·Work·Science의 늦게 끝난 비동기 응답은 사용자가 이미 이동한 다른 대화나 연구 화면을 덮지 않습니다. 키체인 직접 호출이 실패해도 서명된 동일 실행 파일의 격리 경로로 복구합니다. Agentlas Science 0.1.33을 정확한 커밋으로 고정합니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.

- **2026-09-18 · v1.2.24 — 완료 검사와 기존 장기 목표의 재개를 고칩니다** — 목표 완료 검사의 실행·사용량 기록을 작업 기록과 분리하고, Antigravity의 실제 시계·브라우저 실행 증거를 완료 검사에 연결합니다. 예전 목표도 같은 대화에서 지속 수행을 명시해 재개하면 기존 조건과 권한을 보존한 채 장기작업 설정을 적용할 수 있습니다. 장기 작업의 대기·재개, 채팅·예약 실행의 연결 경합, 원래 대화의 자동 보고서 표시를 고친 미공개 1.2.20–1.2.23 수정도 포함합니다. Source readiness does not prove a public installer or update feed; the Releases page stays the authority. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
...
- **2026-09-17 · v1.2.15 — 전체 액세스 승인·스티어링 기록·긴 탭을 바로잡습니다** — One·Work의 전체 액세스가 브라우저·MCP·결제 동작까지 해제되어 `Allow once` 칩이 나타나지 않고, 기존 Goal도 권한 변경 뒤 새 권한으로 재인가됩니다. 스티어링 뒤 늦은 도구 응답은 빨간 실패가 아니라 중단으로 기록되며, 긴 브라우저·파일 탭 제목은 깨지지 않습니다. `ref=e349` 브라우저 참조도 실행 전에 정규화하고, 로컬 unsigned 후보 패키징은 호출 모델이나 셸 환경변수에 의존하지 않습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-16 · v1.2.14 — 대화·Goal·브라우저 장기 실행을 하나로 이어갑니다** — One·Work가 자동 Goal로 전환되어도 사용자가 보낸 대화를 남기고, 중단한 Goal은 직접 편집하고 재개할 수 있습니다. 직접 선택한 로컬 팀과 Goal revision별 컨트롤러 계보를 지원하며, Firm·Swarm·기억 정리의 제어 단계에서 AGY를 막던 불필요한 MCP 전송을 제거했습니다. 전체 액세스 Browser 작업은 반복 승인 없이 실제 탭을 패널에 띄워 계속 조작합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-15 · v1.2.13 — 고른 모델로 실행하고, 실제 도구를 패널에서 봅니다** — Team·Firm·Workforce·Science가 사용자가 고른 모델과 쓰기 권한을 그대로 사용하고, 로컬 소형 모델부터 프로덕션 모델까지 각 모델 크기에 맞춰 요청합니다. Antigravity를 잘못 막던 실행 전 격리 관문을 없앴고, 장기 웹 작업은 처음 URL과 브라우저 연결을 유지합니다. 실행 중인 브라우저·화면 조작·터미널·MCP 결과는 One과 Work의 공용 패널에 자동으로 표시됩니다. 하위 에이전트의 원시 Surface 프로토콜은 완성 답변으로 노출하지 않으며, 내장 로컬 실행은 `Agentlas Local · On-device`, 구형 Ollama는 별도 공급자로 표시합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-15 · v1.2.12 — 오래 켜 둬도 조용하고, 닫으면 실제로 끝납니다** — 보이지 않는 작업 화면·랜딩·캡처·로컬 모델·브라우저·CLI·데몬의 수명주기를 정리했습니다. 기존 프로필은 시작 때 과거 메모리를 다시 훑지 않고, DB를 압축·재구축해도 처리 위치를 잃지 않습니다. 약 130MB 임베딩은 보통 작업 중 다시 읽지 않도록 마지막 사용 뒤 1시간 유지합니다. 로컬 모델 기본 선택은 모델 행에서 켜고 끄며 실제 가속 상태와 맞춥니다. 종료된 일회성 자동화가 가짜 ON이 되지 않고, 업데이트 준비가 멈추면 45초 뒤 정상 종료하며, Windows 프로세스 조회 실패는 원장을 지우지 않고 다음 주기에 재시도합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-15 · v1.2.11 — 연구자가 준 표가 논문에 번호 붙은 표로 들어갑니다** — 지금까지 Science 는 데이터 표를 원고에 묶을 길이 원리적으로 없었습니다(결속에 화면 캡처가 필수였고 캡처는 차트에만 생겼습니다). 오너 데이터 실측에서 결속 218건 중 표 0건. 이제 표의 근거는 그림이 아니라 정확한 행과 그것을 만든 실행이며, 표 전용 출처 관문과 다이제스트로 고정합니다. 열 이름은 내부 키가 아니라 사람이 읽는 이름으로 찍습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-15 · v1.2.10 — Antigravity 로 고른 연구가 시작조차 못 하던 것, 한도 기준 두 벌, 실행 폭주 차단기** — agy 자신의 보호 경계에 막힌 도구 호출 한 건이 실행 전체를 승인 거부로 죽이고, 권한으로는 열 수 없는 경계에 "권한을 올리세요"를 안내하던 것을 고칩니다. 사용량 한도 판정이 역할 풀 90%·실행 선택 100% 로 갈려 있던 것을 100 하나로 모았습니다. 한 런타임이 1분 안에 거듭 실패하면 잠시 실행을 멈춰, 하네스를 잘못 짜도 기계가 타지 않습니다. Science 0.1.31 동반: 폴더 화면 잘림, 멈춘 이유 빈칸, 원고별 1단·2단, 넓은 표를 줄이지 않고 가로로. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.9 — Science 원고가 학술지 모양에 가까워집니다: 표 캡션·한국어 표/그림 라벨·인쇄용 차트·DOCX 수식·경고 PDF 내려받기·분석 결정 원장과 동반 3종** — 표 캡션을 국문 형식으로도 읽고 없으면 경고합니다. 한국어 원고는 표/그림 라벨을 한국어로, 차트는 제목 없이 지면 크기 글자로 다시 그리고 제목 속 식별자를 지웁니다. 수식이 든 원고도 DOCX 가 만들어지고, 조판 경고가 있는 PDF 도 내려받을 수 있습니다. 통계 엔진 1.11.0 은 분석 결정 원장과 HC3·부트스트랩·순열 동반을 자동으로 붙입니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.8 — Science 원고 그림을 편집해도 검증이 이어지고, 검증에 화면을 열 필요가 없으며, 작업 블록이 One 과 같은 규칙으로 읽힙니다** — 제목·색을 고친 차트도 데이터가 같으면 실행 계보를 이어받고, Vega 차트는 검증 때 화면 없이 캡처합니다. "도구 unknown" 줄과 30줄짜리 "생각함" 이 사라지고, 원고 실패 칩·"새 원고 시작" 단추가 진행 상태를 따릅니다. 규정 페이지 EUC-KR 판별, 페이월 안내 거절, 같은 거절 3회면 연구자에게 묻기. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.7 — Science 연구가 실행 중에도 모델을 바꾸고, 각주와 저자 병기를 조판하며, 도구 실패가 사람 말로 옵니다** — 루프 중 모델 변경은 다음 턴부터 적용되고 "지금 적용"이 있습니다. 각주([^key])·국문(영문) 저자·소속·ORCID 지면 표기, 도구 실패 24종에 사람 문장과 다음 행동, 핵심 도구 69개 한국어 키워드, 연구 디렉터 계약에 도구 지도. Work 는 MCP 연결이 끊겨도 같은 자리에서 다시 붙고, 질문 답과 목표 정지 이유·모델 전환 시점을 알립니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.6 — Science 원고가 논문답게 조판되고, 단 구성을 원고마다 고릅니다** — 넓은 표는 두 단에 걸치고 번호 있는 캡션 줄이 표 캡션이 되며, 일반 렌더는 늘 초안 표식이고 깨끗한 PDF 는 제출 번들에서만 나온다는 규칙을 도구가 설명합니다. 원고별 1단/2단 선택(PDF 설정·render 도구). 자동 목표 "변화 없음" 반복을 3턴에서 멈추고, 게임·앱·UI 는 그래픽 품질이 합격 기준입니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.5 — Science 연구가 챗만으로 저널 프로파일과 그림까지 갑니다** — 저널 정체성 확인 도구(자율 정책이면 즉시, 아니면 화면 경로 안내), 그림 결속 사슬과 원고 표식 문법을 도구 설명에, 설치 안내에 분자 시각화 범위(도킹 미지원), 역할 풀에 모델 목록이 빈 이유(코덱스 로그인 전). This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-14 · v1.2.4 — Science 작업 블록이 무엇에 썼는지 보여주고, 로컬 모델 도구 반복 실행이 오류 없이 됩니다** — Science 챗 작업 블록에 "무엇에 썼는가" 한 줄(마스킹, 160자), 로컬 MCP 연결 확인 12초·화면 못 보는 모델의 스크린샷 제외·캡처 요구 면제(4과제 3회 12/12), Work 재시작 막다른 길·승인 문구·역할 단추 라벨, Science 기각 가설 막다른 길 수리. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-13 · v1.2.3 — 로컬 비전 모델을 멀티모달로 등록하고, Science 원고가 학회 논문 모양으로 조판됩니다** — 비전 프로젝터(mmproj)가 있는 로컬 모델은 함께 설치·로드되어 이미지 입력을 받고 대시보드 멀티모달 자리에 올 수 있습니다. 로컬 모델 탐색은 필터 있는 표로. Science 원고 기본 조판은 두 단 10pt 절 위계. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-13 · v1.2.2 — Science 연구가 승인 없이 끝까지 갑니다** — Science 연구 서비스 0.1.20 을 싣습니다: "연구 중지" 뒤 "연구 계속" 이 영원히 거절되던 막다른 길, 결과 검토·후속 분석 계획·재시작 재개를 자율 정책이 덮음, 원고 청사진이 비교 논문 1편부터, 후속 계획으로 실행 전환. 자동 도구 모드에서 URL·"브라우저로" 요청은 판정 없이 Agentlas 브라우저를 붙입니다(로컬 4B 모델 실측 4/4 완주). This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-13 · v1.2.1 — agy가 다시 돕니다** — 사용자가 Antigravity에 직접 등록한 같은 이름의 MCP 서버 하나 때문에 agy 실행 전체가 거절되고 다른 런타임으로 넘어가던 것을 고칩니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-13 · v1.2.0 — 로컬 모델·자동화·화면 개편** — 앱이 직접 관리하는 llama.cpp 로컬 모델과 Hugging Face 허브, One 자연어 자동화와 자동 팀 구성, 작업공간 즉시 시작, 얇아진 브라우저와 문서 패널, 아이콘 중심 허브 카드와 바로잡은 조직도, Science 우측 패널·원고 출력 개편이 들어갑니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-12 · v1.1.17 — 처음 켠 맥에서도 연결이 끝납니다** — 시스템 Node가 없는 맥에서 CLI 로그인 명령이 실패하던 것을 고쳤고, 프로젝트 화면에서 Hub 에이전트를 바로 붙일 수 있습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-12 · v1.1.16 — Science가 agy로 실제로 돕니다** — 예전 실행이 남긴 도구 설정 때문에 Science 연구가 시작도 못 하던 두 자리를 고쳤습니다. 로컬·내 키 실행이 도구 8회에서 끊기던 제한도 없앴습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-11 · v1.1.15 — 설치된 CLI를 다시 찾습니다** — 앱을 아이콘으로 실행하면 Antigravity(agy) 같은 설치된 CLI를 실행 직전 확인에서 "없다"고 거절해 Science와 Work의 실행이 시작되지 않던 문제를 고칩니다. 확인과 실행이 같은 설치 위치를 보도록 한 곳에서 맞췄습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-11 · v1.1.14 — Science 반응 속도** — Science 화면이 무엇을 물어도 설치된 확장 전체를 다시 검사하던 것을 멈춰, 조회가 108ms에서 6.6ms로 줄었습니다. 낡은 폴더가 같은 판 번호를 붙잡고 있어 정식 발행본을 설치할 수 없던 막다른 길과, 근거 하나를 찾으려 대화를 전부 훑던 경로도 함께 고쳤습니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-10 · v1.1.13 — 목표 분류 실패 복원** — 목표 분류 후보가 시간 초과되면 다음 오케스트레이터를 시도하며, 분류 후보가 모두 실패해도 실제 작업은 일반 실행으로 계속합니다. 반복되면 모델 역할의 연결과 사용 한도를 확인하도록 화면에서 안내합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-10 · v1.1.12 — 중복 앱 실행과 모바일 연결 복구** — 미리보기 MCP의 Node 실행 모드 누락을 수정하고, 업데이트 복구 중에도 앱 한 개만 실행되도록 잠금을 유지합니다. 모바일 데몬의 GUI 의존성을 분리하고 실제 준비 상태를 확인한 뒤 연결 소유권을 인계합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
- **2026-09-09 · v1.1.11 — Electron 자식 실행 재기동 루프 차단** — 도구 훅·키체인·프로바이더 실행이 Electron 앱을 새 창으로 잘못 다시 여는 경로를 Node 모드로 고정하고, 시작 PID·부모 PID·인자·모드 흔적을 남깁니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
...
- **2026-08-18 · v1.0.23 — Cloud publish keeps machine-local state at home** — Cloud publish excludes private per-machine .agentlas project state, auto-trims the package to Agent Cloud limits before the identity hash is computed, and turns deliberate server refusals into plain one-sentence answers instead of raw HTTP bodies. Escalation decides staffing from what actually happened in the run, not from words in the request. The mobile bridge can publish to the Hub and price it from a phone; per-work-order rent ships with project toggles and a hard gate, a day-lease dialog on two channels, agent picker search, and the revived bundle-contract gate. Agent pricing and rental in this historical release have since been retired; public Hub publication and calls now use the free Hub contract. This release binds Agentlas OS v1.2.11 at b1e98e8c01b1cd54cb84fd41e244b46f58ff6e6a. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
...
- **2026-06-29 · v0.4.4 BYOK Build pricing** — Desktop Build now treats local
  ...
  At the time, Hub Network calls were billed after quote and confirmation.
  That billing has since been retired; public Hub calls now use the free Hub
  contract. This release also removes local absolute paths and
  realistic-looking fake keys from
  public source files.
