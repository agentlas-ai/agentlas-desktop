...
- **2026-08-12 · v0.9.91 — a model never inherits another model's effort** — A chat pinned to Spark can no longer inherit `max` from a different Codex model and fail before producing an answer. Desktop resolves the exact selected model's live effort profile and provider default before launch, shows only that model's supported levels in the chat menu, and reads those capabilities from the same Codex account home used by the child CLI. This release binds Agentlas OS v1.1.111 at ee1f23911f378b6d521e64d89713c4ef15eb38e9. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
