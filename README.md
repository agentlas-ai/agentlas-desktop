...
- **2026-09-10 · v1.1.12 — 중복 앱 실행·모바일 복구·목표 분류 복원** — 미리보기 MCP의 Node 실행 모드 누락을 수정하고, 업데이트 복구 중에도 앱 한 개만 실행되도록 잠금을 유지합니다. 모바일 데몬의 GUI 의존성을 분리하고 실제 준비 상태를 확인한 뒤 연결 소유권을 인계합니다. 목표 분류 후보가 시간 초과되면 다음 오케스트레이터를 시도하며, 분류 후보가 모두 실패해도 실제 작업은 일반 실행으로 계속합니다. This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d. Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`. Source readiness does not prove a public installer or update feed; the Releases page stays the authority.
