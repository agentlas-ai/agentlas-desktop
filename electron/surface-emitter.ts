...
export const SURFACE_PROTOCOL = [
  ...
  '  },',
  '  "data": { "items": { "type": "table", "rows": [] }, "sales": { "type": "table", "rows": [], "flint": { "chart_spec": { "chartType": "Bar Chart", "encodings": { "x": "category", "y": "value" } }, "options": { "addTooltips": true } } } },',
  '  "widgets": [ { "type": "table", "data": "items" }, { "type": "chart", "data": "sales" } ],',
  ...
  "- For a chart widget, put a bounded Flint input under the referenced table dataset as `flint` (`chart_spec` + optional `semantic_types`, `theme_spec`, and `options`). Use inline rows only; Desktop compiles it offline and Web compiles the same input on the server.",
  "- Keep Flint chart data to at most 2,000 rows and 64 columns, and never use a remote data URL or executable expression.",
