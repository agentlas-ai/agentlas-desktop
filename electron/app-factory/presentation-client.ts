...
export function artifactPresentationClient(config: { token: string; bundleDigest: string }): string {
  ...
  const save=async()=>{
    ...
    saving=(async()=>{
      try{
        ...
        if(result.status==='conflict'){blocked=true;notice('다른 화면에서 저장된 입력이 있습니다. 현재 입력을 보존했어요. 다시 열어 최신 상태를 확인해 주세요.');return;}
      ...
      }catch{dirty=true;notice('입력 상태를 저장하지 못했어요. 현재 화면을 유지해 주세요.');}
    })();
  ...
  }).catch(()=>{blocked=true;notice('저장된 입력 상태를 읽지 못했어요. 현재 입력은 그대로 유지됩니다.');});
  ...
  source.addEventListener('build-failed',()=>notice('새 버전을 준비하지 못했어요. 이전 정상 버전을 표시하고 있습니다.'));
})();`;
}
