// Stormbreaker 슈퍼바이저 — "앱에서 뭘 하든 자동 실행".
//
// 모든 채팅 실행(invoke)을 Hephaestus 의 견고-실행 규율(scope lock → route → evidence/review
// gate)로 감싼다. 비차단·오프라인-안전·실패-무해: 엔진/Python 이 없거나 토글이 꺼져 있으면
// 조용히 no-op 하고, 켜져 있으면 실제 엔진을 호출해 그 판단을 타임라인에 표출한다.
//
// 핵심: 호출자(runner 경로)를 절대 블록하지 않는다. 가용성 프로브(python 탐색)는 첫 호출에서
// 비용이 들 수 있으므로(특히 python 부재 머신) 전부 백그라운드에서 동시 수행하고, 핸들은 즉시
// 반환한다. 메인 실행 경로(검증된 러너)는 절대 대체하지 않는다 — 감독만 한다.
...
/**
 * 채팅 실행을 Stormbreaker 로 감독한다(비차단). 토글이 꺼져 있으면 null.
 * 토글이 켜져 있으면 즉시 핸들을 반환하고, 엔진 가용성 확인·scope-lock·라우팅 평가는
 * 백그라운드에서 동시 수행한다(엔진 부재면 아무 이벤트도 내지 않음).
 */
export function superviseStormbreaker(opts: {
  ...
  const routeP = readyP
    .then(async (ok) => {
      if (!ok || opts.signal?.aborted) return;
      opts.emit({
        name: "🛡️ Stormbreaker · scope-lock",
        result:
      ...
      if (res?.json && !opts.signal?.aborted) {
        opts.emit({ name: "🛡️ Stormbreaker · route", result: summarizeRoute(asObj(res.json)) });
      }
    })
    .catch(() => {
      /* 비차단 */
    ...
    finish: async (fin) => {
      ...
      try {
        ...
        if (producesArtifacts && fin?.workspace) {
          ...
          opts.emit({ name: "🛡️ Stormbreaker · final-gate", result: summarizeGate(asObj(scan.json)) });
        } else {
          opts.emit({
            name: "🛡️ Stormbreaker · final-gate",
