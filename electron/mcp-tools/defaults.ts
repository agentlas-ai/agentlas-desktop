...
// 사용자 동의 없이 실행하지 않는다.
import { installFromCatalog, listInstalledServers, refreshInstalledCatalogServer, setServerEnabled } from "./registry";
import { refreshDedicatedPluginToolCatalog } from "./catalog";
...
import { AGENTLAS_AGENT_MAIL_CATALOG_ID } from "../agent-mail/mcp-server";
import { AGENTLAS_ONE_TEAM_CATALOG_ID } from "../one/team-mcp-server";
...
export const DEFAULT_MCP_CATALOG_IDS = [
  ...
  AGENTLAS_AGENT_MAIL_CATALOG_ID,
  AGENTLAS_ONE_TEAM_CATALOG_ID,
] as const;

export function ensureDefaultMcpPluginsInstalled(): void {
  const dedicated = refreshDedicatedPluginToolCatalog();
  const availableDedicated = new Set(dedicated.installed);
  if (dedicated.failures.length > 0) {
    console.warn("[mcp-defaults] dedicated plugin tool declarations unavailable", dedicated.failures);
  }
  try {
  ...
  // A plugin tool row is user-selected, so discovery never installs it or
  // turns it on. Existing rows receive the current verified launch while their
  // enabled choice stays intact. A removed/corrupt/unsupported release is
  // disabled before this run can serialize its old command.
  for (const server of listInstalledServers()) {
    if (!isDedicatedPluginToolLaunch(server)) continue;
    if (server.catalogId && availableDedicated.has(server.catalogId)) {
      try { refreshInstalledCatalogServer(server.catalogId); } catch (err) {
        console.error(`[mcp-defaults] dedicated plugin ${server.catalogId} refresh failed:`, err);
        if (server.enabled) setServerEnabled(server.id, false);
      }
  ...
  for (const catalogId of DEFAULT_MCP_CATALOG_IDS) {
    try {
      if (!installed.has(catalogId)) {
        ...
        catalogId === AGENTLAS_AGENT_MAIL_CATALOG_ID ||
        catalogId === AGENTLAS_ONE_TEAM_CATALOG_ID
      ) {
