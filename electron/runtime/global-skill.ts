...
export const GLOBAL_CONNECTION_SKILL = [
  ...
  "For image, video, and audio GENERATION the engine is ALREADY resolved for you by Agentlas — do NOT probe engines, do NOT test which CLI exists, and do NOT narrate an engine-selection ladder to the user. Just read these env values and act:",
  "  - AGENTLAS_MULTIMODAL_IMAGE_PROVIDER / VIDEO_PROVIDER / AUDIO_PROVIDER = the chosen engine id, and the matching AGENTLAS_MULTIMODAL_*_READY = \"1\" (ready) or \"0\" (nothing connected).",
  "  - If READY=\"1\": use that engine DIRECTLY and immediately, no explanation of the choice. Mapping: 'codex-cli-image' -> run `codex exec -s workspace-write --skip-git-repo-check` with its built-in image_gen tool (keyless, no API key). 'nanobanana-image' -> run the `agy` CLI keyless image generation (Nano Banana / Gemini image). Any '*-image' / '*-video' / '*-audio' API or cloud provider -> its key is ALREADY in your env; call that provider's API. Produce the asset; do not describe the ladder.",
  "  - If READY=\"0\": the user has NO engine connected for that modality. Do NOT open a browser to create accounts or sign up for media generation, and do NOT flail. Emit EXACTLY the line `<<agentlas-multimodal-setup>>` on its own line, add one short sentence telling the user to open multimodal settings and pick or connect an engine, then STOP.",
  "",
