...
import {
  ...
  systemTimeMcpServerPath,
  systemTimeMcpSourceDigest,
...
export function hasPinnedSiteAgentAppExecutable(server: InstalledMcpServer): boolean {
  ...
  if (server.catalogId !== AGENTLAS_SYSTEM_TIME_CATALOG_ID) return false;
  if (server.transport !== "stdio" || server.url !== null ||
      path.resolve(server.command ?? "") !== path.resolve(process.execPath) || server.args.length !== 1 ||
      path.resolve(server.args[0]) !== path.resolve(systemTimeMcpServerPath())) return false;
  const source = readStableRegularFile(server.args[0], 64 * 1024);
...
export function validateSiteAgentAppMcpConfigBytes(input: {
  ...
  if (!path.isAbsolute(configPath) || bytes.length < 1 || bytes.length > SITE_AGENT_APP_MCP_CONFIG_MAX_BYTES ||
