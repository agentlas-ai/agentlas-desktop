...
export class ScienceArtifactPublicationValidator {
  ...
  validate(input: ValidateScienceArtifactForPublicationInput): ValidateScienceArtifactForPublicationResult {
    ...
    const prior = this.store.listArtifactValidationReceipts(input.projectId, input.artifactId, input.artifactVersion)
      .find((receipt) => receipt.status === "verified"
    ...
    const challengeSha256 = sha256(canonicalJson({
      schema: "agentlas.science-publication-validation-challenge/v1",
    ...
    const requestId = input.requestId ?? stableUuid(`science-publication-validation:v1:${input.projectId}:${input.artifactId}:${input.artifactVersion}:${preview.sha256}`);
...
export function sciencePublicationValidationRequestId(seed: string): string {
  return stableUuid(`science-publication-validation-request:v1:${seed || randomUUID()}`);
}
