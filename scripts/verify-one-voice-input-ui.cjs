...
assert.match(helper, /Fn or the Globe key twice/);
assert.match(helper, /Windows key \+ H/);
assert.match(helper, /does not turn on the microphone or save audio here/);
assert.match(helper, /Review the dictated text before sending it yourself/);
...
assert.match(helper, /Return to composer/);
