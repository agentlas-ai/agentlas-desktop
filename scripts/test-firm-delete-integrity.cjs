...
async function main() {
  ...
  assert.equal(db.pragma("user_version", { simple: true }), require("../package.json").agentlasUpdateCompatibility.targetSchemaVersion, "v48 store must migrate through the current canonical schema");
    const ceoFk = db
