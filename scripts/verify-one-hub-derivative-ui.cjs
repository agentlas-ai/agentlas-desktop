...
assert.match(card, /Your files and memories stay out of the public draft|내 파일과 기억은 공개 초안에 넣지 않아요/, "candidate copy must disclose the zero-source-byte boundary in beginner language");
assert.match(card, /creates only a new public description and basic structure|공개 설명과 기본 구조만 새로 준비합니다/, "candidate copy must describe the generated-only draft truthfully without developer jargon");
assert.doesNotMatch(card, /allowlisted material|허용 목록만 복사/, "UI must not claim private source files were copied");
assert.match(card, /Publishing access, your right to publish, credit availability, and fees still need review|게시 권한·내가 올릴 권리·크레딧 기능·수수료는 아직 확인이 필요합니다/);
...
assert.match(handoff, /확인 필요|Needs review/, "publish prerequisites must remain visibly unresolved");
...
assert.match(handoff, /Not published|아직 게시 안 됨/);
assert.match(handoff, /Earnings are not guaranteed|수익은 보장되지 않습니다/);
