...
- **Agentlas App Builder** (`architecture/manifest.ts`, slug `agentlas-app-builder`) —
  ...
  workflow goals into generated local web apps registered in Apps, keeps generated
  surfaces/tools as support evidence, and uses design-reference patterns without
  publishing third-party service names as product copy.
