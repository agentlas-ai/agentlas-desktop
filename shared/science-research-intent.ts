...
const INTENTS: Readonly<Record<string, ScienceLabResearchIntent>> = Object.freeze({
  ...
  "paleontology-evidence": {
    labId: "paleontology-evidence",
    neededWhen: "When a paleobiological question depends on fossil occurrences, taxonomic assignments, named rock units, temporal overlap, geographic coverage, preservation gaps, or the strength of stratigraphic support for a historical claim.",
    notWhen: "Do not use this Lab to infer unobserved anatomy, soft tissue, endogenous DNA, a genome sequence, developmental viability, hatching, or de-extinction feasibility from catalog occurrences or temporal proximity alone.",
    liveDecision: "Which exact fossil records and age intervals support the proposed taxon, place, and time claim; which records conflict or remain unresolved; and whether the evidence is sufficient for a bounded paleontological conclusion or requires specimen-level follow-up.",
    userGoal: "Move from a broad dinosaur or fossil question to a row-auditable evidence set, visualize interval uncertainty without fabricating precision, compare candidate taxa on defensible support, and carry exact limitations into the next analysis and manuscript.",
    requiredInputs: [
      "one exact successful PBDB catalog run with immutable provider bytes and source hashes",
      "taxon concept, rank, synonym policy, geographic scope, and geological time question",
      "decision target such as occurrence support, temporal overlap, candidate comparison, or specimen follow-up",
      "for a de-extinction audit, an explicit choice between actual biological revival and comparative proxy research plus exact source runs for every non-missing criterion",
    ],
    clarifyingQuestions: [
      q("deextinction-objective", "Is the objective literal biological revival, or a comparative proxy programme about morphology, proteins, development, or ancestral-state models?", "single-select", "Blocking for every de-extinction request because the two objectives use different gates and must never share a feasibility score."),
      q("deextinction-no-go", "If the four biological evidence gates are not met, should the audit return an explicit NO-GO for actual revival and continue only with a separately labeled proxy-research route?", "single-select", "Blocking before candidate comparison; fossil coverage cannot compensate for missing DNA, genome, viable-cell, or embryo-surrogate evidence."),
      q("paleontology-question", "Is the decision about verified occurrence, taxonomic identity, stratigraphic range, geographic distribution, candidate comparison, or specimen-level follow-up?", "single-select", "Always blocking when a broad request such as 'most recoverable dinosaur' has not been translated into a fossil-evidence question."),
      q("taxon-concept", "Which accepted taxon, rank, synonyms, and uncertain or reidentified records should be included?", "free-text", "Blocking before aggregation because taxonomic revisions can change both record membership and the apparent temporal range."),
      q("age-policy", "Should overlapping age intervals be compared by full preserved bounds, named stages, formations, or a declared display midpoint?", "multi-select", "Blocking before ranking or chart interpretation; a midpoint may be shown only as a display summary and never replaces the source interval."),
      q("claim-target", "Which conclusion must the evidence support, and is any requested morphology or molecular claim backed by an independent specimen, imaging, biomolecular, or genomic source?", "free-text", "Blocking whenever the requested conclusion exceeds fossil occurrence or stratigraphic support."),
    ],
    rendering: {
      mustShow: [
        "every included occurrence with stable occurrence, collection, taxon, and reference identities",
        "early and late age bounds as intervals with named stage, formation, member, lithology, environment, locality, and explicit missingness",
        "the exact provider query, retrieval time, raw-response and normalized-result hashes, page boundaries, and catalog run identity",
        "a row-auditable table plus an interval Figure whose marks trace back to source rows and do not imply midpoint precision",
        "support, conflict, ambiguity, and coverage-gap counts alongside all exclusions and unresolved taxonomic assignments",
      ],
      researcherActions: [
        "inspect a plotted interval and open the exact PBDB occurrence, collection, taxon, and reference fields behind it",
        "filter or compare taxa, formations, stages, localities, and age-overlap policies without overwriting the preserved catalog run",
        "exclude or flag a disputed record with a recorded rationale and save the decision as a successor analysis rather than silently deleting it",
        "promote an approved table or Figure version to the manuscript or create a specimen, morphology, imaging, or biomolecular follow-up plan",
      ],
      aiInspectionSignals: [
        "taxonomic synonym, rank, identification, or accepted-name conflict",
        "age interval reversal, missing bounds, broad uncertainty, or mismatch between named stage and numeric range",
        "formation, locality, coordinate, lithology, environment, or reference missingness that limits interpretation",
        "duplicate occurrences, collection clustering, uneven temporal or geographic sampling, and apparent range extensions driven by one record",
        "claim escalation from fossil occurrence to morphology, DNA, genome reconstruction, embryo viability, hatching, or de-extinction",
      ],
      claimBoundaries: [
        "A PBDB occurrence is literature-derived evidence that a taxon was reported from a collection; it is not direct reinspection of a specimen and does not independently validate the identification.",
        "Fossil occurrence and stratigraphic overlap do not establish morphology, preserved soft tissue, endogenous DNA, recoverable genome sequence, developmental compatibility, embryo viability, hatching, or de-extinction feasibility.",
        "An interval midpoint is only a visualization summary. It must never be reported as the fossil's measured age or used to create false temporal precision.",
        "Ranking a taxon by catalog support ranks the available fossil evidence under declared criteria; it does not rank biological resurrectability.",
      ],
    },
    nextActions: [
      next("literal-revival-request", "run-fixed-four-gate-deextinction-audit", "artifact", null, "The researcher needs an auditable stop decision, not a cosmetic score that averages away missing biological prerequisites."),
      next("literal-revival-no-go", "open-separate-comparative-proxy-research-decision", "human-decision", null, "A stopped biological-revival branch may lead to useful proxy research only after the objective and claims are explicitly changed.", true),
      next("taxonomic-or-age-conflict", "open-exact-occurrence-collection-and-reference-records", "artifact", null, "The researcher must resolve the records that can materially change the inferred stratigraphic range before aggregation."),
      next("sampling-or-coverage-gap", "create-targeted-museum-field-or-literature-verification-plan", "analysis-plan", null, "A missing or uneven catalog signal should produce a falsifiable evidence-acquisition plan rather than an absence claim.", true),
      next("candidate-comparison-approved", "freeze-ranking-criteria-and-run-sensitivity-comparisons", "analysis-plan", null, "Candidate ordering must be stable across declared taxonomic, temporal, geographic, and missingness policies before it is reported.", true),
      next("morphology-or-molecular-claim-requested", "require-independent-specimen-imaging-or-biomolecular-evidence", "human-decision", null, "The paleontology catalog cannot provide the evidence class required for anatomy, DNA, genome, embryo, hatching, or de-extinction claims.", true),
      next("stratigraphic-result-reviewed", "bind-interval-figure-occurrence-table-source-closure-and-limitations", "manuscript", null, "The paper must preserve the exact supporting rows, interval semantics, provenance, and unsupported-claim boundary with the reported conclusion.", true),
    ],
    manuscript: {
      roles: ["figure", "table", "supplement"],
      requirements: [
        "taxon concept, accepted names, ranks, synonym and disputed-identification policy, geographic scope, and reproducible provider query",
        "occurrence and collection identifiers, named stratigraphic units, full early and late bounds, interval convention, exclusions, missingness, and sampling limitations",
        "provider version, retrieval timestamp, source and run hashes, reference linkage, analysis policy, and row-to-mark provenance",
        "caption and Results language that describes interval support without midpoint precision and explicitly separates fossil evidence from morphology, DNA, genome, embryo, hatching, and de-extinction claims",
      ],
    },
  },
  ...
  "comparative-genomics": {
    ...
    neededWhen: "When a gene-family question requires version-receipted extant sequences, explicit orthology/paralogy evidence, a provider alignment, and a rooted gene tree before any ancestral hypothesis is considered.",
    notWhen: "Do not use a provider gene tree as a species tree, call an extant-lineage MRCA a named extinct species, or infer an extinct genome, chromosome, phenotype, embryo, or hatching path.",
    liveDecision: "Whether the sampled extant sequences, duplication history, alignment support, and target MRCA are sufficient for the next sensitivity analysis or require the study to stop.",
    userGoal: "Inspect the exact extant sequence evidence and gene-family topology, trace every branch to provider bytes, and decide whether a bounded hypothetical ancestral-state study is defensible.",
    requiredInputs: ["extant Ensembl gene ID and reference species", "extant prune taxon and sequence type", "versioned provider release and exact response receipts"],
    clarifyingQuestions: [
      q("comparative-target", "Which extant gene ID, reference species, sequence type, and extant taxon define the comparison?", "free-text", "Always blocking before retrieval."),
      q("orthology-policy", "Should duplications and one-to-many relationships stop the study, or remain as explicitly reviewed alternatives?", "single-select", "Blocking before selecting a representative sequence or proposing ASR."),
      q("ancestral-node", "Which reproducibly defined extant-taxon MRCA is the target, and is it stable across planned alignment/model/topology alternatives?", "free-text", "Blocking before any ancestral-state materialization."),
    ],
    rendering: {
      mustShow: ["rooted provider gene tree with gene IDs, taxa, branch support, and duplication/gene-split nodes", "alignment length, gap and missingness QC by extant sequence", "provider release, request URLs, raw-response hashes, and ResearchRun", "observed versus inferred versus hypothetical evidence boundary"],
      researcherActions: ["select and inspect a taxon, gene copy, branch, or duplication", "open the exact source response and alignment QC row", "choose a target MRCA for a later sensitivity plan", "bind the exact tree figure and QC table to a manuscript"],
      aiInspectionSignals: ["duplication or unresolved paralogy", "low branch support", "high gap or missing fraction", "provider gene-tree versus species-tree confusion"],
      claimBoundaries: ["Extant sequence records are observed provider data; orthology, alignment, and topology are inferred.", "A future ancestral sequence remains hypothetical and cannot be relabelled as a named extinct-species genome."],
    },
    nextActions: [
      next("duplication-or-low-support", "stop-representative-selection-and-request-orthology-review", "human-decision", null, "The selected gene copy and target node are not yet uniquely justified.", true),
      next("tree-and-alignment-reviewed", "create-preregistered-local-alignment-model-topology-sensitivity-plan", "analysis-plan", null, "The provider tree is a starting hypothesis, not the final inferential analysis.", true),
      next("figure-and-table-approved", "bind-tree-qc-table-and-provider-receipts", "manuscript", null, "The paper must preserve the exact tree, alignment summary, release, and limitations.", true),
    ],
    manuscript: { roles: ["figure", "table", "supplement"], requirements: ["provider release, gene IDs, taxa, and exact response hashes", "alignment and branch-support diagnostics", "duplication/paralogy and gene-tree/species-tree limitations", "explicit statement that no extinct-species genome was reconstructed"] },
  },
  "genomics-variants": {
