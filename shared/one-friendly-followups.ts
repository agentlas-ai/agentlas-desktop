...
export function oneFriendlyFollowupProtocol(locale: "ko" | "en"): string {
  ...
  return locale === "ko"
    ? `완료된 작업을 "완료했습니다"로만 끝내지 마세요. 사용자가 다음 가치를 바로 얻도록, 실제로 가능한 후속 행동을 최대 3개 골라 ${ONE_FOLLOWUPS_OPEN_FENCE} JSON ${ONE_FOLLOWUPS_CLOSE_FENCE} 블록으로 Surface 바로 앞에 넣으세요. 우선순위는 (1) 바로 써보기, (2) 결과가 있는 메뉴 열기, (3) 다듬기·재사용·공유 준비입니다. 외부 전송·게시·구매는 실행하지 말고 준비 단계만 제안하세요. 가능한 intent는 try_result, open_asset, refine_result, reuse_result, prepare_share뿐입니다. 각 intent는 각각 runnable, inspectable, editable, reusable, shareable capability가 있을 때만 쓰세요. open_asset에는 instruction을 넣지 말고, 나머지는 같은 Task에서 실행할 짧은 instruction을 넣으세요. 예: ${example}`
    : `Do not end completed work with only "done." Help the user get the next value immediately by adding up to three real follow-up actions in a ${ONE_FOLLOWUPS_OPEN_FENCE} JSON ${ONE_FOLLOWUPS_CLOSE_FENCE} block immediately before the Surface. Prioritize (1) try it now, (2) open where it lives, then (3) refine, reuse, or prepare to share. Never send, publish, purchase, or otherwise cause an external effect; offer preparation only. Allowed intents are try_result, open_asset, refine_result, reuse_result, and prepare_share. Use them only with runnable, inspectable, editable, reusable, and shareable capability respectively. open_asset has no instruction; every other action needs a short instruction for the same Task. Example: ${example}`;
}
