...
3. Starting values for peak fits and York fits are the caller's responsibility: read them off the data first, then check `summary.converged`, `stopReason`, `at-bound` parameters, and χ²/ndf before quoting any parameter.
   For a spectrum fit with a mapped `sigma_column` whose cells are all empty, omit `uncertainty_model` so the recorded result uses Poisson weighting with a warning. A partly empty measured uncertainty column is not silently filled; explicit `column` weighting requires complete values.
