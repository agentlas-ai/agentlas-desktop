...
- A credential-free stdio MCP server exposing `search_simbad_catalog`, `analyze_astrometric_kinematics`, `analyze_light_curve_periodicity`, and `describe_astronomy_capabilities`.
...
The strongest period is a local maximum on the requested finite grid, not a detection probability. The tool reports sampling-window power and cadence/baseline warnings but deliberately does not calculate a false-alarm probability, trial-factor correction, period confidence interval, red-noise model, detrending, multi-harmonic model, or transit template. The single-sinusoid result therefore requires independent domain review before a periodicity claim.
